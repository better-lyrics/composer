...
const PreviewLine: React.FC<{
	...
	const isLineCurrent =
		granularity === "line"
			? !!linePosition
			: activePositions.some((p) => p.lineIndex === lineIndex);
		...
		>
			{words.length > 0
				? words.map((word, wordIdx) => {
						...
						const wordPosition = activePositions.find(
							(p) => p.lineIndex === lineIndex && p.wordIndex === wordIdx,
						);
