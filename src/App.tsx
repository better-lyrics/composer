...
const AppContent: React.FC = () => {
	...
	const shortcuts: Shortcut[] = useMemo(
		() => [
			...
			{ key: "4", ctrl: true, action: () => setActiveTab("preview"), description: "Preview" },
			{ key: "5", ctrl: true, action: () => setActiveTab("export"), description: "Export" },
		],
		[setActiveTab],
	);
	...
	return (
		<div className="flex h-screen flex-col bg-composer-bg text-composer-text">
			<header className="flex items-center justify-between p-4 border-b border-composer-border select-none">
				<h1 className="text-xl font-bold">Composer</h1>
			</header>
			<TabBar />
			<main className="relative flex-1 overflow-hidden">
				<div
					className={`absolute inset-0 flex flex-col ${activeTab === "import" ? "visible" : "invisible"}`}
				>
					<ImportPanel />
				</div>
